        ...
        var evaluation = Enumerable.Range(1, 100)
            .Aggregate((
                ...
                (previousEvaluation, depth) => 
                    timer.MillisecondsElapsedThisTurn > 2500
                        ? previousEvaluation
                        : board.GetLegalMoves()
                            .Select(move => (
                                    move: move, 
                                    // score_tt: board.MakeMove(move, board => AlphaBeta(board, lastDepthMoveScore.transpositionTable, depth, () => false))
                                    score_tt: board.MakeMove(move, board => AlphaBeta(board, previousEvaluation.transpositionTable, depth, () => timer.MillisecondsElapsedThisTurn > 2500))
                                ))
                            .Select(evaluation => (
                                evaluation.move,
